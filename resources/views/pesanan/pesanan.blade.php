...
@section('content')
                    ...
                    <div class="card data-tables">
                        ...
                        <div
                            ...
                            <div class="toolbar">
                                <div class="row">
                                    <div class="col-9">
                                        <h4 class="card-title col-5">Data Pesanan</h4>
                                    </div>
                                </div>
                                </br>
                                    ...
                                    <tbody>
                                        @foreach ($pesanan as $index => $row)
                                            <tr>
                                                ...
                                                <td class="text-center">{{ ($row->status == 0) ? "Pesanan Baru" : "Pesanan Selesai" }}
                                                </td>
                                                <td class="text-center">
                                                    <a href="{{ route('pesanan.edit', $row->id) }}" type="button"
                                                        class="btn btn-warning"><i class="fa fa-edit"
                                                            title="Edit"></i></a>
                                                    <button type="button" class="btn btn-info" data-toggle="modal"
                                                        data-target="#modalSelesai{{ $row->id }}"
                                                        title="Pesan Selesai" {{($row->status == 1) ? "disabled" : ""}} {{(auth()->user()->role != "kasir") ? "disabled" : ""}}><i
    ...
    @foreach ($pesanan as $index => $row)
        <!-- Modal Hapus -->
        <div class="modal modal-alert py-5" id="modalSelesai{{ $row->id }}" tabindex="-1" role="dialog"
            aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content rounded-4 shadow">
                    <form method="post" action="{{ route('pesanan.destroy', $row->id) }}" enctype="multipart/form-data">
                        @method('DELETE')
                        @csrf
                        <div class="modal-body p-4 text-center">
                            <h5 class="mb-0">Apakah anda ingin menyelesaikan pesanan ini?</h5>
                            <p class="mb-0">Tekan tombol BACK untuk membatalkan.</p>
                        </div>
